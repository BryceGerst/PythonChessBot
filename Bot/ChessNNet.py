...
# https://github.com/suragnair/alpha-zero-general/blob/master/othello/tensorflow/OthelloNNet.py
...
class ChessNNet:
    def __init__(self, args):
        ...
        with self.graph.as_default(): 
            self.input_boards = tf.compat.v1.placeholder(tf.float32, shape=[None, input_planes, self.board_x, self.board_y])    # s: batch_size x board_x x board_y
            self.dropout = tf.compat.v1.placeholder(tf.float32)
            self.isTraining = tf.compat.v1.placeholder(tf.bool, name="is_training")

            x_image = tf.reshape(self.input_boards, [-1, input_planes, self.board_x, self.board_y, 1])                    # batch_size  x board_x x board_y x 1
    ...
    def conv2d(self, x, out_channels, padding):
      return tf.layers.conv3d(x, out_channels, kernel_size=[3,3], padding=padding, use_bias=False)
...
class ResNet:
    def __init__(self, args):
        ...
        with self.graph.as_default(): 
            ...
            x_image = tf.reshape(self.input_boards, [-1, input_planes, self.board_x, self.board_y, 1])                    # batch_size  x board_x x board_y x 1
            x_image = tf.keras.layers.Conv2D(x_image, args.num_channels, kernel_size=(3, 3), strides=(1, 1),name='conv',padding='same',use_bias=False)
            ...
            policy = tf.keras.layers.Conv2D(residual_tower, 2,kernel_size=(1, 1), strides=(1, 1),name='pi',padding='same',use_bias=False)
            ...
            value = tf.keras.layers.Conv2D(residual_tower, 1,kernel_size=(1, 1), strides=(1, 1),name='v',padding='same',use_bias=False)
    ...
    def residual_block(self,inputLayer, filters,kernel_size,stage,block):
        ...
        residual_layer = tf.keras.layers.Conv2D(inputLayer, filters,kernel_size=(kernel_size, kernel_size), strides=(1, 1),name=conv_name+'2a',padding='same',use_bias=False)
        ...
        residual_layer = tf.keras.layers.Conv2D(residual_layer, filters,kernel_size=(kernel_size, kernel_size), strides=(1, 1),name=conv_name+'2b',padding='same',use_bias=False)
    ...
    def calculate_loss(self):
        ...
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            self.train_step = tf.compat.v1.AdamOptimizer(self.args.lr).minimize(self.total_loss)
